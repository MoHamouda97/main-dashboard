...
export class PaymentMethodComponent implements OnInit, OnChanges {
  ...
  //#region 
    ...
    displayData(data: any) {
      this.payMethodForm.get('PayMethod').setValue(data[0].PayMethod);
      this.payMethodForm.get('Method').setValue(data[0].Method);
      this.payMethodForm.get('Notes').setValue(data[0].Notes);

      document.getElementById('PayMethod').setAttribute('disabled', 'true');
    }
  ...
  //#region 
    ...
    returnArrayFromFormValues() {
      let data = [];
      data.push(["PayMethod", this.payMethodForm.get("PayMethod").value]);
      data.push(["Method", this.payMethodForm.get("Method").value]);
      data.push(["Notes", (this.payMethodForm.get("Notes").value == null) ? "" : this.payMethodForm.get("Notes").value]);
      return data;
    }
   ...
   const isReset = setInterval(() => {
      if (typeof (changes.isReset) == 'undefined') {
        ...
        if (this.isReset) {
          ...
          document.getElementById('PayMethod').removeAttribute('disabled');        
        }
    ...
    const isReporting = setInterval(() => {
      if (typeof (changes.isReporting) == 'undefined') {
        ...
        if (this.isReporting) {
          this.returnID.emit(this.payMethodForm.get('PayMethod').value);
        }
