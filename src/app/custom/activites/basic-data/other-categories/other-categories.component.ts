...
export class OtherCategoriesComponent implements OnInit, OnChanges {
  // form var
  ...
  //#region 
    ...
    displayData(data: any) {
      this.otherCategoryForm.get('CategoryCode').setValue(data[0].CategoryCode);
      this.otherCategoryForm.get('Description').setValue(data[0].Description);
  ...
  //#region 
    ...
    returnArrayFromFormValues() {
      let data = [];
      data.push(["CategoryCode", this.otherCategoryForm.get("CategoryCode").value]);
      data.push(["Description", this.otherCategoryForm.get("Description").value]);
      return data;
    }
   ...
   const isReset = setInterval(() => {
    ...
    const isReporting = setInterval(() => {
      if (typeof (changes.isReporting) == 'undefined') {
        ...
        if (this.isReporting) {
          this.returnID.emit(this.otherCategoryForm.get('CategoryCode').value);
        }
