...
export class GridComponent implements OnInit, OnChanges {
    ...
    this.binding.autoTax.subscribe(
      res => {
        if (res != null) {  
          ...
          if (Array.isArray(this.taxArr[0])) this.taxArr.shift();
  ...
  //#region     
    ...
    addRow() { // add grid row according to grid type
      ...
      this.gridLingth = Object.keys(this.data[0]).length;
    }
          ...
          if (Array.isArray(newVal)) {
            if (newVal.length > 0) {
              for (let i = 0; i <= newVal.length - 1; i++) {
                ...
                if (newVal[i].hasOwnProperty("ItemCode")) {
                  if (newVal[i]["ItemCode"].slice(0, 8) == this.system[0].Glb_VAT_ItemCode.slice(0, 8) && this.isAutoTax) {
                    this.taxArr.push(newVal[i]);
                    Array.isArray(this.taxArr[0]) && this.taxArr.shift()
                  }                  
            ...
            if(changes.gridDataGrid.currentValue.hasOwnProperty("ItemCode")) {
