...
export class CommissionPariodComponent implements OnInit, OnChanges {
  ...
  //#region 
    ...
    returnArrayFromFormValues() {
      let data = [];
      data.push(["PeriodCode", this.comissionForm.get("PeriodCode").value]);
      data.push(["PeriodDescription", this.comissionForm.get("PeriodDescription").value]);
      data.push(["StartDate", this.formatDate(this.comissionForm.get("StartDate").value)]);
      data.push(["EndDate", this.formatDate(this.comissionForm.get("EndDate").value)]);
      return data;
    }
