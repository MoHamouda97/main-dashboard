...
export class CustomerCategoriesComponent implements OnInit, OnChanges {
  ...
  //#region 
    ...
    returnArrayFromFormValues() {
      let data = [];
      data.push(["CategoryCode", this.customerCategoryForm.get("CategoryCode").value]);
      data.push(["Description", this.customerCategoryForm.get("Description").value]);
      data.push(["DiscountRatio", this.customerCategoryForm.get("DiscountRatio").value]);
      return data;
    }
