...
export class EntryTypesComponent implements OnInit, OnDestroy, OnChanges {
  ...
  //#region 
    ...
    returnArrayFromFormValues() {
      let data = [];
      data.push(["TransCode", this.entryTypesForm.get("TransCode").value]);
      data.push(["TransName", this.entryTypesForm.get("TransName").value]);
      data.push(["Notes", (this.entryTypesForm.get("Notes").value == null) ? "" : this.entryTypesForm.get("Notes").value]);
      return data;
    }
