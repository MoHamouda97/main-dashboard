...
export class BanksComponent implements OnInit, OnChanges {
  ...
  //#region 
    ...
    returnArrayFromFormValues() {
      let data = [];
      data.push(["BankNum", this.banksForm.get("BankNum").value]);
      data.push(["BankName", this.banksForm.get("BankName").value]);
      data.push(["OurBankAccount", this.banksForm.get("OurBankAccount").value]);
      data.push(["BankAddress", this.banksForm.get("BankAddress").value]);
      data.push(["Notes", (this.banksForm.get("Notes").value == null) ? "" : this.banksForm.get("Notes").value]);
      return data;
    }
